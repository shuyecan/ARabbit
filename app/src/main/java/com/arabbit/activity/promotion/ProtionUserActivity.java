...
import com.arabbit.activity.Userpage.MyGiftInfoActivity;
import com.arabbit.activity.Userpage.SeeUserCardActivity;
import com.arabbit.adapter.ProUserAdapter;
...
public class ProtionUserActivity extends BaseActivity {
    ...
    List<UserHadproEntity.ListsBean> mList = new ArrayList();

    public void proList(int newpage) {
        pt_id = getIntent().getStringExtra("pt_id");
        try {
            model.getProUser(pt_id,newpage, new IModelResult<UserHadproEntity>() {
                @Override
                public void OnSuccess(UserHadproEntity userEntities) {
                    mList.clear();
                    if (!CommonUtils.isNull(userEntities)) {
                        List<UserHadproEntity.ListsBean> newList = userEntities.getLists();
                        if (!CommonUtils.isNull(newList)) {
                            mList.addAll(newList);
                        }
                        accountPage = userEntities.getTotal_page();
    ...
    ProUserAdapter mAdapter;

    private void initList() {
        mAdapter = new ProUserAdapter(mActivity, mList);
