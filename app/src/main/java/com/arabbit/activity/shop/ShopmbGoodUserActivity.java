...
import com.arabbit.adapter.ShopmbUserAdapter;
...
public class ShopmbGoodUserActivity extends BaseActivity implements AdapterView.OnItemClickListener,ShopmbUserAdapter.Callback {
    ...
    public void userShopmbList(final int page) {
        mbgood_id = getIntent().getStringExtra("mbgood_id");
        try {
            model.getUserShopmbGoodList(mbgood_id,page,new IModelResult<HadShopmbEntity>() {
                @Override
                public void OnSuccess(HadShopmbEntity hadShopmbEntities) {
                    ...
                    if (!CommonUtils.isNull(hadShopmbEntities)) {
                        ...
                        accountPage = hadShopmbEntities.getTotal_page();
    ...
    ShopmbUserAdapter mAdapter;

    private void initList() {
        mAdapter = new ShopmbUserAdapter(mActivity, mList,this);
